/**
 * Function: 全文模糊搜索
 *   - 标签 0.3 + 注释 0.2 + 内容 0.5 权重
 *   - 搜索结果高亮展示、点击跳转定位节点
 * Dependencies: tree.js, storage.js
 */
'use strict';
...
const SearchMgr = (() => {
  ...
  function close() { document.getElementById('modal-search').classList.add('hidden'); }
  ...
  async function doSearch(kw) {
    ...
    for (const node of nodes) {
      ...
      results.push({ node, score, excerpt });
    }
    ...
    for (const { node, excerpt } of results) {
      ...
      `;
      item.addEventListener('click', () => { TreeMgr.setActive(node.id); close(); });
  ...
  function init() {
    ...
    let debT;
    inp.addEventListener('input', () => { clearTimeout(debT); debT = setTimeout(() => doSearch(inp.value), 200); });
    document.getElementById('modal-search').addEventListener('click', e => { if (e.target === e.currentTarget) close(); });
  }
