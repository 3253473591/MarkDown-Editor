/**
 * Function (功能): 
 *   - 统一数据持久化层：localStorage + IndexedDB 双层存储策略
 *   - 节点内容、背景图、备份、折叠状态、隐藏状态的读写管理
 *   - 存储配额监控与清理（>5MB 转 IDB，>10MB 强制清理）
 * Dependencies (依赖): 无（底层基础设施，被其他模块依赖）
 * Bug Fix Guide (Bug 修复提示):
 *   若数据丢失、存储溢出未提示、IndexedDB 报错、背景图无法保存，
 *   仅需上传此文件给 LLM。
 *   若与存储相关的业务逻辑异常（如节点内容不同步），需额外上传 tree.js。
 */
...
const LS_META   = 'md_meta';       // tree structure + small hot data
...
const StorageMgr = {
  saveMeta(meta) {
    ...
    localStorage.setItem(LS_META, json);
  },
  loadMeta() {
  ...
  async saveNodeContent(id, content) {
    ...
    if (bytes > SIZE_THRESHOLD) {
      ...
      try {
        localStorage.setItem('nd_' + id, content);
      } catch {
        // quota exceeded → fallback to IDB
  ...
  /* ── Background image ── */
  async saveBg(blob) {
    await idbPut(STORE_BG, { key: 'bg', blob });
  },
  async loadBg() {
    const rec = await idbGet(STORE_BG, 'bg');
    return rec ? rec.blob : null;
  },
  async clearBg() {
    await idbDelete(STORE_BG, 'bg');
  },

  /* ── Backups ── */
  async saveBackup(metaSnapshot) {
    const ts = Date.now();
    await idbPut(STORE_BACKUPS, { ts, meta: metaSnapshot });
  },
  async listBackups() {
  ...
  async cleanOrphanNodes(liveIds) {
    ...
    for (const rec of all) {
      if (!liveIds.has(rec.id)) {
        freed += strBytes(JSON.stringify(rec));
        await idbDelete(STORE_NODES, rec.id);
      }
    }
    // also clean localStorage orphans
    for (const k of Object.keys(localStorage)) {
      if (k.startsWith('nd_') && !liveIds.has(k.slice(3))) {
        freed += strBytes(localStorage[k]);
  ...
  async cleanOldBackups(keep) {
    ...
    for (let i = keep; i < sorted.length; i++) {
      freed += strBytes(JSON.stringify(sorted[i]));
      await idbDelete(STORE_BACKUPS, sorted[i].ts);
    }
    return freed;
  },
  ...
  /* ── Quota monitor ── */
  async checkQuota() {
    const size = await idbSize();
    return { size, warn: size > IDB_WARN, critical: size > IDB_FORCE };
  },

  /* ── Size helpers (exposed) ── */
